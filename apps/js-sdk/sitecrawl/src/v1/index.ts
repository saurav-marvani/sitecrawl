...
/**
 * Configuration interface for SitecrawlApp.
 * @param apiKey - Optional API key for authentication.
 * @param apiUrl - Optional base URL of the API; defaults to 'https://api.sitecrawl.dev'.
 */
export interface SitecrawlAppConfig {
  apiKey?: string | null;
  apiUrl?: string | null;
}

/**
 * Metadata for a Sitecrawl document.
 * Includes various optional properties for document metadata.
 */
export interface SitecrawlDocumentMetadata {
  title?: string;
  description?: string;
  language?: string;
  keywords?: string;
...
/**
 * Document interface for Sitecrawl.
 * Represents a document retrieved or processed by Sitecrawl.
 */
export interface SitecrawlDocument<T = any, ActionsSchema extends (ActionsResult | never) = never> {
  url?: string;
  markdown?: string;
  html?: string;
  rawHtml?: string;
  ...
  json?: T;
  screenshot?: string;
  metadata?: SitecrawlDocumentMetadata;
 ...
 */
export interface ScrapeResponse<LLMResult = any, ActionsSchema extends (ActionsResult | never) = never> extends SitecrawlDocument<LLMResult, ActionsSchema> {
  success: true;
  warning?: string;
  error?: string;
}
...
export interface CrawlStatusResponse {
  ...
  expiresAt: Date;
  next?: string;
  data: SitecrawlDocument<undefined>[];
};
...
/**
 * Custom error class for Sitecrawl.
...
/**
 * Main class for interacting with the Sitecrawl API.
 * Provides methods for scraping, searching, crawling, and mapping web content.
 */
export default class SitecrawlApp {
  public apiKey: string;
  public apiUrl: string;
  public version: string =  "1.25.1";
  
  private isCloudService(url: string): boolean {
    return url.includes('api.sitecrawl.dev');
  }
  ...
  /**
   * Initializes a new instance of the SitecrawlApp class.
   * @param config - Configuration options for the SitecrawlApp instance.
   */
  constructor({ apiKey = null, apiUrl = null }: SitecrawlAppConfig) {
    const baseUrl = apiUrl || "https://api.sitecrawl.dev";
    
    if (this.isCloudService(baseUrl) && typeof apiKey !== "string") {
      throw new SitecrawlError("No API key provided", 401);
    }
  ...
  /**
   * Scrapes a URL using the Sitecrawl API.
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (responseData.success) {
          ...
          throw new SitecrawlError(`Failed to scrape URL. Error: ${responseData.error}`, response.status);
        }
      } else {
  ...
  /**
   * Searches using the Sitecrawl API and optionally scrapes the results.
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (responseData.success) {
          return {
            success: true,
            data: responseData.data as SitecrawlDocument<any>[],
            warning: responseData.warning,
          };
        } else {
          throw new SitecrawlError(`Failed to search. Error: ${responseData.error}`, response.status);
        }
      } else {
      ...
      if (error.response?.data?.error) {
        throw new SitecrawlError(`Request failed with status code ${error.response.status}. Error: ${error.response.data.error} ${error.response.data.details ? ` - ${JSON.stringify(error.response.data.details)}` : ''}`, error.response.status);
      } else {
        throw new SitecrawlError(error.message, 500);
      }
    }
  ...
  /**
   * Initiates a crawl job for a URL using the Sitecrawl API.
  ...
  /**
   * Checks the status of a crawl job using the Sitecrawl API.
  ...
  async checkCrawlStatus(id?: string, getAllData = false, nextURL?: string, skip?: number, limit?: number): Promise<CrawlStatusResponse | ErrorResponse> {
    if (!id) {
      throw new SitecrawlError("No crawl ID provided", 400);
    }
  ...
  /**
   * Cancels a crawl job using the Sitecrawl API.
   * @param id - The ID of the crawl operation.
  ...
  async crawlUrlAndWatch(
    ...
    throw new SitecrawlError("Crawl job failed to start", 400);
  }

  /**
   * Maps a URL using the Sitecrawl API.
  ...
  /**
   * Initiates a batch scrape job for multiple URLs using the Sitecrawl API.
  ...
  async batchScrapeUrlsAndWatch(
    ...
    throw new SitecrawlError("Batch scrape job failed to start", 400);
  }

  /**
   * Checks the status of a batch scrape job using the Sitecrawl API.
  ...
  async checkBatchScrapeStatus(id?: string, getAllData = false, nextURL?: string, skip?: number, limit?: number): Promise<BatchScrapeStatusResponse | ErrorResponse> {
    if (!id) {
      throw new SitecrawlError("No batch scrape ID provided", 400);
    }
  ...
  /**
   * Extracts information from URLs using the Sitecrawl API.
   ...
   * Review https://docs.sitecrawl.dev/developer-guides/usage-guides/choosing-the-data-extractor to find a replacement.
   */
  async extract<T extends zt.ZodSchema = any>(urls?: string[], params?: ExtractParams<T>): Promise<ExtractResponse<zt.infer<T>> | ErrorResponse> {
    ...
    try {
        ...
        do {
          ...
          if (extractStatus.status === "completed") {
            if (extractStatus.success) {
              ...
              throw new SitecrawlError(`Failed to extract data. Error: ${extractStatus.error}`, statusResponse.status);
            }
          } else if (extractStatus.status === "failed" || extractStatus.status === "cancelled") {
            throw new SitecrawlError(`Extract job ${extractStatus.status}. Error: ${extractStatus.error}`, statusResponse.status);
          }
      ...
      throw new SitecrawlError(error.message, 500, error.response?.data?.details);
    }
    return { success: false, error: "Internal server error."};
  }

  /**
   * Initiates an asynchronous extract job for a URL using the Sitecrawl API.
  ...
  async monitorJobStatus(
    ...
    while (true) {
      try {
        ...
        if (statusResponse.status === 200) {
          ...
          if (statusData.status === "completed") {
            if ("data" in statusData) {
              ...
              throw new SitecrawlError("Crawl job completed but no data was returned", 500);
            }
          } else if (
        ...
        throw new SitecrawlError(error, 500);
      }
    }
  }
  ...
  async deepResearch(
    ...
    try {
      ...
      if (!response.id) {
        throw new SitecrawlError(`Failed to start research. No job ID returned.`, 500);
      }
  ...
  async checkDeepResearchStatus(id: string): Promise<DeepResearchStatusResponse | ErrorResponse> {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        throw new SitecrawlError("Deep research job not found", 404);
      } else {
  ...
  async generateLLMsText(url: string, params?: GenerateLLMsTextParams): Promise<GenerateLLMsTextStatusResponse | ErrorResponse> {
    try {
      ...
      if (!response.id) {
        throw new SitecrawlError(`Failed to start LLMs.txt generation. No job ID returned.`, 500);
      }
  ...
  async checkGenerateLLMsTextStatus(id: string): Promise<GenerateLLMsTextStatusResponse | ErrorResponse> {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        throw new SitecrawlError("LLMs.txt generation job not found", 404);
      } else {
...
interface CrawlWatcherEvents {
  document: CustomEvent<SitecrawlDocument<undefined>>,
  done: CustomEvent<{
...
export class CrawlWatcher extends TypedEventTarget<CrawlWatcherEvents> {
  ...
  public data: SitecrawlDocument<undefined>[];
  public status: CrawlStatusResponse["status"];
  public id: string;

  constructor(id: string, app: SitecrawlApp) {
    super();
    this.id = id;
