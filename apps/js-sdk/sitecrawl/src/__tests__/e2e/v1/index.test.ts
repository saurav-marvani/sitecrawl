import SitecrawlApp, { type CrawlParams, type CrawlResponse, type CrawlStatusResponse, type MapResponse, type ScrapeResponse } from '../../../index';
...
const API_URL = process.env.API_URL ?? "https://api.sitecrawl.dev";

describe('SitecrawlApp E2E Tests', () => {
  test.concurrent('should throw error for no API key only for cloud service', async () => {
    if (API_URL.includes('api.sitecrawl.dev')) {
      // Should throw for cloud service
      expect(() => {
        new SitecrawlApp({ apiKey: null, apiUrl: API_URL });
  ...
  test.concurrent('should throw error for invalid API key on scrape', async () => {
    if (API_URL.includes('api.sitecrawl.dev')) {
      const invalidApp = new SitecrawlApp({ apiKey: "invalid_api_key", apiUrl: API_URL });
      await expect(invalidApp.scrapeUrl('https://sitecrawl-test-site.vercel.app')).rejects.toThrow("Unexpected error occurred while trying to scrape URL. Status code: 401");
    } else {
      const invalidApp = new SitecrawlApp({ apiKey: "invalid_api_key", apiUrl: API_URL });
      await expect(invalidApp.scrapeUrl('https://sitecrawl-test-site.vercel.app')).resolves.not.toThrow();
    }
  });

  test.concurrent('should throw error for unsupported URL on scrape', async () => {
    const app = new SitecrawlApp({ apiKey: TEST_API_KEY, apiUrl: API_URL });
  ...
  test.concurrent('should return successful response for valid scrape', async () => {
    ...
    const response = await app.scrapeUrl('https://sitecrawl-test-site.vercel.app');
    if (!response.success) {
  ...
  test.concurrent('should throw error for invalid API key on crawl', async () => {
    if (API_URL.includes('api.sitecrawl.dev')) {
      const invalidApp = new SitecrawlApp({ apiKey: "invalid_api_key", apiUrl: API_URL });
      await expect(invalidApp.crawlUrl('https://sitecrawl-test-site.vercel.app')).rejects.toThrow("Request failed with status code 401");
    } else {
      const invalidApp = new SitecrawlApp({ apiKey: "invalid_api_key", apiUrl: API_URL });
      await expect(invalidApp.crawlUrl('https://sitecrawl-test-site.vercel.app')).resolves.not.toThrow();
    }
  });

  test.concurrent('should return successful response for crawl and wait for completion', async () => {
    const app = new SitecrawlApp({ apiKey: TEST_API_KEY, apiUrl: API_URL });
    const response = await app.crawlUrl('https://sitecrawl-test-site.vercel.app', {}, 30) as CrawlStatusResponse;
  ...
  test.concurrent('should return successful response for crawl with options and wait for completion', async () => {
    const app = new SitecrawlApp({ apiKey: TEST_API_KEY, apiUrl: API_URL });
    const response = await app.crawlUrl('https://sitecrawl-test-site.vercel.app', {
      excludePaths: ['blog/*'],
      includePaths: ['/'],
      maxDepth: 2,
      ignoreSitemap: true,
  ...
  test.concurrent('should handle idempotency key for crawl', async () => {
    ...
    const response = await app.asyncCrawlUrl('https://sitecrawl-test-site.vercel.app', {}, uniqueIdempotencyKey) as CrawlResponse;
    expect(response).not.toBeNull();
    expect(response.id).toBeDefined();

    await expect(app.crawlUrl('https://sitecrawl-test-site.vercel.app', {}, 2, uniqueIdempotencyKey)).rejects.toThrow("Request failed with status code 409");
  });

  test.concurrent('should check crawl status', async () => {
    const app = new SitecrawlApp({ apiKey: TEST_API_KEY, apiUrl: API_URL });
    const response = await app.asyncCrawlUrl('https://sitecrawl.dev', { limit: 20, scrapeOptions: { formats: ['markdown', 'html', 'rawHtml', 'screenshot', 'links'] } } as CrawlParams) as CrawlResponse;
    expect(response).not.toBeNull();
    expect(response.id).toBeDefined();
  ...
  test.concurrent('should throw error for invalid API key on map', async () => {
    if (API_URL.includes('api.sitecrawl.dev')) {
      const invalidApp = new SitecrawlApp({ apiKey: "invalid_api_key", apiUrl: API_URL });
      await expect(invalidApp.mapUrl('https://sitecrawl-test-site.vercel.app')).rejects.toThrow("Request failed with status code 401");
    } else {
      const invalidApp = new SitecrawlApp({ apiKey: "invalid_api_key", apiUrl: API_URL });
      await expect(invalidApp.mapUrl('https://sitecrawl-test-site.vercel.app')).resolves.not.toThrow();
    }
  });
  ...
  test.concurrent('should return successful response for valid map', async () => {
    const app = new SitecrawlApp({ apiKey: TEST_API_KEY, apiUrl: API_URL }); const response = await app.mapUrl('https://sitecrawl-test-site.vercel.app') as MapResponse;
    expect(response).not.toBeNull();

    expect(response.links?.length).toBeGreaterThan(0);
    expect(response.links?.[0]).toContain("https://");
    const filteredLinks = response.links?.filter((link: string) => link.includes("sitecrawl-test-site.vercel.app"));
  ...
  test('should search with string query', async () => {
    const app = new SitecrawlApp({ apiUrl: API_URL, apiKey: TEST_API_KEY });
    const response = await app.search("sitecrawl");
  ...
  test('should handle invalid API key for search', async () => {
    const app = new SitecrawlApp({ apiUrl: API_URL, apiKey: "invalid_api_key" });
