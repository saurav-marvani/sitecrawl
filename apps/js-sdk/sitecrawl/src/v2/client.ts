 ...
 */
export interface SitecrawlClientOptions {
  /** API key (falls back to SITECRAWL_API_KEY). */
  apiKey?: string | null;
  /** API base URL (falls back to SITECRAWL_API_URL or https://api.sitecrawl.dev). */
  apiUrl?: string | null;
...
export type SitecrawlClientInput = SitecrawlClientOptions | string;

/**
 * Sitecrawl v2 client. Provides typed access to all v2 endpoints and utilities.
 */

export class SitecrawlClient {
   ...
   */
  constructor(options: SitecrawlClientInput = {}) {
    const opts: SitecrawlClientOptions =
      typeof options === "string" ? { apiKey: options } : options;

    const apiKey = (opts.apiKey ?? process.env.SITECRAWL_API_KEY ?? "").trim();
    const apiUrl = (opts.apiUrl ?? process.env.SITECRAWL_API_URL ?? "https://api.sitecrawl.dev").replace(/\/$/, "");
   ...
   * Example: `sitecrawl.research.searchPapers("diffusion models")`.
   */
...
export default SitecrawlClient;
