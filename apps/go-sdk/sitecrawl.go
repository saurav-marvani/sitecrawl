// Package sitecrawl provides a Go client for the Sitecrawl v2 web scraping API.
//
...
//	)
//
// Or let the client read the SITECRAWL_API_KEY environment variable:
//
//	client, err := sitecrawl.NewClient()
package sitecrawl
...
// Client is the Sitecrawl v2 API client.
type Client struct {
	http *httpClient
}

// NewClient creates a new Sitecrawl client.
//
// The API key is resolved in order from:
//  1. [option.WithAPIKey]
//  2. SITECRAWL_API_KEY environment variable
//
// The API URL defaults to https://api.sitecrawl.dev and can be overridden
// with [option.WithAPIURL] or the SITECRAWL_API_URL environment variable.
func NewClient(opts ...option.RequestOption) (*Client, error) {
	...
	if apiKey == "" {
		apiKey = strings.TrimSpace(os.Getenv("SITECRAWL_API_KEY"))
	}
	...
	apiURL := cfg.APIURL
	if apiURL == "" {
		apiURL = os.Getenv("SITECRAWL_API_URL")
	}
	if apiURL == "" {
...
func (c *Client) Scrape(ctx context.Context, url string, opts *ScrapeOptions) (*Document, error) {
	if url == "" {
		return nil, &SitecrawlError{Message: "URL is required"}
	}
...
func (c *Client) Interact(ctx context.Context, jobID, code string, params *InteractParams) (*BrowserExecuteResponse, error) {
	if jobID == "" {
		return nil, &SitecrawlError{Message: "job ID is required"}
	}
	if code == "" {
		return nil, &SitecrawlError{Message: "code is required"}
	}
	...
	if err := json.Unmarshal(raw, &resp); err != nil {
		return nil, &SitecrawlError{Message: fmt.Sprintf("failed to decode response: %v", err)}
	}
	return &resp, nil
}
...
func (c *Client) StartCrawl(ctx context.Context, url string, opts *CrawlOptions) (*CrawlResponse, error) {
	...
	if opts != nil && opts.Limit != nil && *opts.Limit <= 0 {
		return nil, &SitecrawlError{Message: "limit must be positive"}
	}
...
func (c *Client) StartBatchScrape(ctx context.Context, urls []string, opts *BatchScrapeOptions) (*BatchScrapeResponse, error) {
	if len(urls) == 0 {
		return nil, &SitecrawlError{Message: "URLs list is required"}
	}
...
func (c *Client) CreateMonitor(ctx context.Context, req *MonitorCreateRequest) (*Monitor, error) {
	if req == nil {
		return nil, &SitecrawlError{Message: "monitor request is required"}
	}
...
func (c *Client) GetMonitor(ctx context.Context, monitorID string) (*Monitor, error) {
	if monitorID == "" {
		return nil, &SitecrawlError{Message: "monitor ID is required"}
	}
...
func (c *Client) UpdateMonitor(ctx context.Context, monitorID string, req *MonitorUpdateRequest) (*Monitor, error) {
	...
	if req == nil {
		return nil, &SitecrawlError{Message: "monitor update request is required"}
	}
...
func (c *Client) DeleteMonitor(ctx context.Context, monitorID string) (bool, error) {
	if monitorID == "" {
		return false, &SitecrawlError{Message: "monitor ID is required"}
	}
	...
	if err := json.Unmarshal(raw, &resp); err != nil {
		return false, &SitecrawlError{Message: fmt.Sprintf("failed to decode response: %v", err)}
	}
...
func (c *Client) GetMonitorCheck(ctx context.Context, monitorID, checkID string, opts *GetMonitorCheckOptions) (*MonitorCheckDetail, error) {
	if monitorID == "" || checkID == "" {
		return nil, &SitecrawlError{Message: "monitor ID and check ID are required"}
	}
...
func (c *Client) Search(ctx context.Context, query string, opts *SearchOptions) (*SearchData, error) {
	if query == "" {
		return nil, &SitecrawlError{Message: "query is required"}
	}
	if opts != nil && opts.Limit != nil && *opts.Limit <= 0 {
		return nil, &SitecrawlError{Message: "limit must be positive"}
	}
...
func (c *Client) InspectPaper(ctx context.Context, paperID string) (*PaperMetadataResponse, error) {
	if paperID == "" {
		return nil, &SitecrawlError{Message: "paper ID is required"}
	}
...
func (c *Client) RelatedPapers(ctx context.Context, paperID, intent string, opts *RelatedPapersOptions) (*SimilarPapersResponse, error) {
	...
	if intent == "" {
		return nil, &SitecrawlError{Message: "intent is required"}
	}
	values := url.Values{}
...
func (c *Client) StartAgent(ctx context.Context, opts *AgentOptions) (*AgentResponse, error) {
	if opts == nil {
		return nil, &SitecrawlError{Message: "agent options are required"}
	}
...
func (c *Client) AgentWithPolling(ctx context.Context, opts *AgentOptions, pollIntervalSec, timeoutSec int) (*AgentStatusResponse, error) {
	...
	if start.ID == "" {
		return nil, &SitecrawlError{Message: "agent start did not return a job ID"}
	}
	...
	return nil, &JobTimeoutError{
		SitecrawlError: SitecrawlError{Message: "agent job timed out"},
		JobID:          start.ID,
...
func (c *Client) BrowserExecute(ctx context.Context, sessionID, code string, params *BrowserExecuteParams) (*BrowserExecuteResponse, error) {
	if sessionID == "" {
		return nil, &SitecrawlError{Message: "session ID is required"}
	}
	if code == "" {
		return nil, &SitecrawlError{Message: "code is required"}
	}
...
func (c *Client) pollCrawl(ctx context.Context, jobID string, pollIntervalSec, timeoutSec int) (*CrawlJob, error) {
	...
	return nil, &JobTimeoutError{
		SitecrawlError: SitecrawlError{Message: "crawl job timed out"},
		JobID:          jobID,
		TimeoutSeconds: timeoutSec,
	}
}
...
func (c *Client) pollBatchScrape(ctx context.Context, jobID string, pollIntervalSec, timeoutSec int) (*BatchScrapeJob, error) {
	...
	return nil, &JobTimeoutError{
		SitecrawlError: SitecrawlError{Message: "batch scrape job timed out"},
		JobID:          jobID,
		TimeoutSeconds: timeoutSec,
	}
}
...
func (c *Client) paginateCrawl(ctx context.Context, job *CrawlJob) (*CrawlJob, error) {
	...
	for current.Next != "" {
		...
		if err := json.Unmarshal(raw, &nextPage); err != nil {
			return nil, &SitecrawlError{Message: fmt.Sprintf("failed to decode pagination response: %v", err)}
		}
...
func extractDataAs[T any](raw json.RawMessage) (*T, error) {
	...
	if err := json.Unmarshal(dataRaw, &result); err != nil {
		return nil, &SitecrawlError{Message: fmt.Sprintf("failed to decode data: %v", err)}
	}
	return &result, nil
}
