...
it('scrapes a URL and returns markdown', function (): void {
    ...
    $result = (new SitecrawlScrape($client))->handle(new Request(['url' => 'https://example.com']));
...
it('surfaces a success:false envelope as an error instead of empty content', function (): void {
    ...
    $result = (new SitecrawlScrape($client))->handle(new Request(['url' => 'https://nosuchdomain.example']));
...
it('resolves the client from the container when none is injected', function (): void {
    ...
    $container->instance(SitecrawlClient::class, fakeSitecrawlClient([
        new Response(200, [], json_encode(['success' => true, 'data' => ['markdown' => '# Via Container']])),
    ]));
    Container::setInstance($container);

    try {
        $result = (new SitecrawlScrape())->handle(new Request(['url' => 'https://example.com']));
    } finally {
...
it('exposes name, description, and a required url parameter', function (): void {
    $tool = new SitecrawlScrape(fakeSitecrawlClient([]));

    expect($tool->name())->toBe('sitecrawl_scrape');
