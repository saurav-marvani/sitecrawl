...
class SitecrawlError(Exception):
    """Base exception for Sitecrawl API errors."""
...
class BadRequestError(SitecrawlError):
    """Raised when the request is invalid (400)."""
    pass



class UnauthorizedError(SitecrawlError):
    """Raised when the request is unauthorized (401)."""
    pass


class PaymentRequiredError(SitecrawlError):
    """Raised when payment is required (402)."""
    pass


class WebsiteNotSupportedError(SitecrawlError):
    """Raised when website is not supported (403)."""
    pass


class RequestTimeoutError(SitecrawlError):
...
class InternalServerError(SitecrawlError):
...
def handle_response_error(response: requests.Response, action: str) -> None:
    ...
    if response.status_code == 400:
        ...
        raise SitecrawlError(message, response.status_code, response)