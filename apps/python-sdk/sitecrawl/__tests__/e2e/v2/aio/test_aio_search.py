...
sitecrawl = AsyncSitecrawl(api_key=os.getenv("API_KEY"), api_url=os.getenv("API_URL"))
...
async def test_async_search_minimal_request():
    results = await sitecrawl.search(
...
async def test_async_search_all_parameters():
    from sitecrawl.types import ScrapeOptions, Location, WaitAction
    schema = {
        "type": "object",
        "properties": {
            "title": {"type": "string"},
...
async def test_async_search_formats_flexibility():
    ...
    results1 = await sitecrawl.search(
    ...
    results2 = await sitecrawl.search(
        query="python programming",
        limit=1,
        scrape_options=ScrapeOptions(
