...
from sitecrawl.v2.utils.http_client_async import AsyncHttpClient
from sitecrawl.v2.utils.http_client import HttpClient
from sitecrawl.v2.methods.aio import batch as aio_batch
...
async def test_scrape_concurrency(monkeypatch):
    ...
    client = AsyncSitecrawlClient(api_key="test", api_url="http://localhost")

    start = time.perf_counter()
    await asyncio.gather(
        client.scrape("https://sitecrawl.dev"),
        client.scrape("https://sitecrawl.dev"),
        client.scrape("https://sitecrawl.dev")
    )
