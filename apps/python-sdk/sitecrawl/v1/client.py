"""
Sitecrawl v1 API Client - Legacy Implementation

This module provides the legacy v1 implementation of the Sitecrawl SDK.
It contains the complete `V1SitecrawlApp` class with all v1 API methods and types
...
Classes:
    - V1SitecrawlApp: Legacy v1 client for interacting with the Sitecrawl API.
    - AsyncV1SitecrawlApp: Async version of the v1 client.
...
# class V1SitecrawlDocumentMetadata(pydantic.BaseModel):
...
class V1SitecrawlDocument(pydantic.BaseModel, Generic[T]):
    """Document retrieved or processed by Sitecrawl."""
...
class V1ScrapeResponse(V1SitecrawlDocument[T], Generic[T]):
...
class V1SitecrawlApp:
    """
    Legacy v1 Sitecrawl client for backward compatibility.
    ...
    def __init__(self, api_key: Optional[str] = None, api_url: Optional[str] = None) -> None:
        """
        Initialize the V1SitecrawlApp instance with API key, API URL.

        Args:
            api_key (Optional[str]): API key for authenticating with the Sitecrawl API.
            api_url (Optional[str]): Base URL for the Sitecrawl API.
        """
        self.api_key = api_key or os.getenv('SITECRAWL_API_KEY')
        self.api_url = api_url or os.getenv('SITECRAWL_API_URL', 'https://api.sitecrawl.dev')
        
        # Only require API key when using cloud service
        if 'api.sitecrawl.dev' in self.api_url and self.api_key is None:
            logger.warning("No API key provided for cloud service")
            raise ValueError('No API key provided')
            
        logger.debug(f"Initialized V1SitecrawlApp with API URL: {self.api_url}")
    ...
    def scrape_url(
        ...
        if response.status_code == 200:
            try:
                ...
                raise Exception('Failed to parse Sitecrawl response as JSON.')
        else:
    ...
    def search(
        ...
        """
        Search for content using Sitecrawl.
        ...
        Returns:
            SearchResponse: Response containing:
                ...
                * data (List[SitecrawlDocument]): Search results
        ...
        **kwargs
        ...
        if response.status_code == 200:
            try:
                ...
                raise Exception(f'Failed to parse Sitecrawl response as JSON.')
    ...
    def check_batch_scrape_status(self, id: str) -> V1BatchScrapeStatusResponse:
        """
        Check the status of a batch scrape job using the Sitecrawl API.
    ...
    def extract(
        ...
        .. deprecated::
            ...
            Review https://docs.sitecrawl.dev/developer-guides/usage-guides/choosing-the-data-extractor
        ...
        try:
            ...
            if response.status_code == 200:
                try:
                    ...
                    raise Exception(f'Failed to parse Sitecrawl response as GenerateLLMsTextStatusResponse: {str(e)}')
...
class V1CrawlWatcher:
    ...
    Attributes:
        ...
        app (V1SitecrawlApp): The V1SitecrawlApp instance
    ...
    """
    def __init__(self, id: str, app: V1SitecrawlApp):
        self.id = id
        self.app = app
...
class AsyncV1SitecrawlApp(V1SitecrawlApp):
    """
    Asynchronous version of V1SitecrawlApp that implements async methods using aiohttp.
    Provides non-blocking alternatives to all V1SitecrawlApp operations.
    """

    def __init__(self, api_key: str, api_url: str = "https://api.sitecrawl.dev"):
    ...
    async def _handle_error(self, response: aiohttp.ClientResponse, action: str) -> None:
        ...
        try:
            ...
            raise aiohttp.ClientError(f'Failed to parse Sitecrawl error response as JSON. Status code: {response.status}')
    ...
    async def search(
            ...
            **kwargs) -> V1SearchResponse:
        """
        Asynchronously search for content using Sitecrawl.
...
class AsyncV1CrawlWatcher(V1CrawlWatcher):
    ...
    """
    def __init__(self, id: str, app: AsyncV1SitecrawlApp):
