defmodule SitecrawlTest do
  use ExUnit.Case

  test "raises when no API key is configured" do
    old = Application.get_env(:sitecrawl, :api_key)
    Application.delete_env(:sitecrawl, :api_key)
    on_exit(fn -> if old, do: Application.put_env(:sitecrawl, :api_key, old) end)

    assert_raise RuntimeError, ~r/Sitecrawl API key not found/, fn ->
      Sitecrawl.get_credit_usage()
    end
  end

  test "does not raise when API key is in application config" do
    Application.put_env(:sitecrawl, :api_key, "test-config-key")
    on_exit(fn -> Application.delete_env(:sitecrawl, :api_key) end)

    result = Sitecrawl.get_queue_status(base_url: "http://localhost:1", retry: false)
  ...
  test "non-bang returns {:error, _} for missing required params" do
    Application.put_env(:sitecrawl, :api_key, "test-key")
    on_exit(fn -> Application.delete_env(:sitecrawl, :api_key) end)

    assert {:error, %NimbleOptions.ValidationError{}} =
             Sitecrawl.scrape_and_extract_from_url([])
  end
  ...
  test "non-bang returns {:error, _} for unknown keys (typo detection)" do
    ...
    assert {:error, %NimbleOptions.ValidationError{message: msg}} =
             Sitecrawl.scrape_and_extract_from_url(url: "https://example.com", typo_option: true)
  ...
  test "non-bang returns {:error, _} for wrong types" do
    ...
    assert {:error, %NimbleOptions.ValidationError{}} =
             Sitecrawl.crawl_urls(url: "https://example.com", limit: "not an integer")
  end
  ...
  test "parse_file returns error tuple when filename is empty" do
    ...
    assert {:error, %ArgumentError{message: msg}} =
             Sitecrawl.parse_file([filename: "", data: "x"])
  ...
  test "parse_file returns error tuple when data is nil" do
    ...
    assert {:error, %ArgumentError{message: msg}} =
             Sitecrawl.parse_file([filename: "doc.pdf", data: nil])
  ...
  test "non-bang returns {:error, %Sitecrawl.Error{}} for API errors" do
    adapter = fn request ->
    ...
    assert {:error, %Sitecrawl.Error{status: 402}} = result
  end

  test "bang raises Sitecrawl.Error for API errors" do
    adapter = fn request ->
      resp = Req.Response.new(
        status: 401,
        headers: %{"content-type" => ["application/json"]},
    ...
    assert_raise Sitecrawl.Error, ~r/Unauthorized/, fn ->
      Sitecrawl.scrape_and_extract_from_url!(
        [url: "https://example.com"],
        api_key: "test-key",
        adapter: adapter
  ...
  test "batch scrape maps redact_pii to redactPII" do
    ...
    assert {:ok, %Req.Response{status: 200}} =
             Sitecrawl.scrape_and_extract_from_urls(
  ...
  test "all expected API functions are defined with bang variants" do
    functions = Sitecrawl.__info__(:functions)
    ...
    for {name, arity} <- expected do
      assert {name, arity} in functions,
             "Expected #{name}/#{arity} to be defined in Sitecrawl"
    end
  end
end
