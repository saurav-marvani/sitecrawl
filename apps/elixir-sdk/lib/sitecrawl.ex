...
defmodule Sitecrawl do
  @moduledoc """
  Auto-generated Sitecrawl API v2 client.

  Generated from the OpenAPI spec at:
  https://raw.githubusercontent.com/sitecrawl/sitecrawl-docs/main/api-reference/v2-openapi.json
  ...
  @type response :: {:ok, Req.Response.t()} | {:error, Exception.t() | Sitecrawl.Error.t()}

  @base_url "https://api.sitecrawl.dev/v2"
  @sdk_origin "elixir-sdk@1.6.1"

  defp client(opts) do
    api_key =
      Keyword.get_lazy(opts, :api_key, fn ->
        Application.get_env(:sitecrawl, :api_key)
      end)
    ...
    |> Req.merge(opts)
    |> Req.Request.append_response_steps(sitecrawl_error_handler: &handle_api_error/1)
  end

  defp handle_api_error({request, %Req.Response{status: status} = response}) when status >= 400 do
    {request, Sitecrawl.Error.exception(status: status, body: response.body)}
  end
  ...
  @crawl_urls_schema NimbleOptions.new!([
    ...
    exclude_paths: [type: {:list, :string}, doc: "URL pathname regex patterns that exclude matching URLs from the crawl. For example, if you set \"excludePaths\": [\"blog/.*\"] for the base URL sitecrawl.dev, any results matching that pattern will be excluded, such as https://www.sitecrawl.dev/blog/sitecrawl-launch-week-1-recap."],
    ignore_query_parameters: [type: :boolean, doc: "Do not re-scrape the same path with different (or none) query parameters"],
    include_paths: [type: {:list, :string}, doc: "URL pathname regex patterns that include matching URLs in the crawl. Only the paths that match the specified patterns will be included in the response. Note: the starting URL is also checked against these patterns — if it does not match, the crawl may return 0 pages. For example, if you set \"includePaths\": [\"blog/.*\"] for the base URL sitecrawl.dev/blog, only pages under /blog/ will be included in the results, such as https://www.sitecrawl.dev/blog/sitecrawl-launch-week-1-recap."],
    ...
    zero_data_retention: [type: :boolean, doc: "If true, this will enable zero data retention for this crawl. To enable this feature, please contact help@sitecrawl.dev"]
  ])
  ...
  @parse_file_schema NimbleOptions.new!([
    ...
    zero_data_retention: [type: :boolean, doc: "If true, this will enable zero data retention for this parse. To enable this feature, please contact help@sitecrawl.dev"]
  ])
  ...
  @scrape_and_extract_from_url_schema NimbleOptions.new!([
    ...
    proxy: [type: {:in, [:basic, :enhanced, :auto]}, doc: "Specifies the type of proxy to use.\n\n - **basic**: Proxies for scraping sites with none to basic anti-bot solutions. Fast and usually works.\n - **enhanced**: Enhanced proxies for scraping sites with advanced anti-bot solutions. Slower, but more reliable on certain sites. Costs up to 5 credits per request.\n - **auto**: Sitecrawl will automatically retry scraping with enhanced proxies if the basic proxy fails. If the retry with enhanced is successful, 5 credits will be billed for the scrape. If the first attempt with basic is successful, only the regular cost will be billed."],
    redact_pii: [type: :boolean, doc: "Redact personally identifiable information from returned content."],
    ...
    skip_tls_verification: [type: :boolean, doc: "Skip TLS certificate verification when making requests."],
    store_in_cache: [type: :boolean, doc: "If true, the page will be stored in the Sitecrawl index and cache. Setting this to false is useful if your scraping activity may have data protection concerns. Using some parameters associated with sensitive scraping (e.g. actions, headers) will force this parameter to be false."],
    ...
    wait_for: [type: :integer, doc: "Specify a delay in milliseconds before fetching the content, allowing the page sufficient time to load. This waiting time is in addition to Sitecrawl's smart wait feature."],
    zero_data_retention: [type: :boolean, doc: "If true, this will enable zero data retention for this scrape. To enable this feature, please contact help@sitecrawl.dev"]
  ])
  ...
  @scrape_and_extract_from_urls_schema NimbleOptions.new!([
    ...
    timeout: [type: :integer, doc: "Timeout in milliseconds for the request. Minimum is 1000 (1 second). Default is 60000 (60 seconds). Maximum is 300000 (300 seconds)."],
    wait_for: [type: :integer, doc: "Specify a delay in milliseconds before fetching the content, allowing the page sufficient time to load. This waiting time is in addition to Sitecrawl's smart wait feature."],
    zero_data_retention: [type: :boolean, doc: "If true, this will enable zero data retention for this batch scrape. To enable this feature, please contact help@sitecrawl.dev"]
  ])
  ...
  @search_and_scrape_schema NimbleOptions.new!([
    ...
    ignore_invalid_urls: [type: :boolean, doc: "Excludes URLs from the search results that are invalid for other Sitecrawl endpoints. This helps reduce errors if you are piping data from search into other Sitecrawl API endpoints."],
