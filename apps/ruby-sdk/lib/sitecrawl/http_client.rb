  ...
  class HttpClient
    ...
    def get_absolute(absolute_url)
      ...
      unless uri.host == base_uri.host && uri.port == base_uri.port && uri.scheme == base_uri.scheme
        raise SitecrawlError, "Absolute URL origin (#{uri.scheme}://#{uri.host}:#{uri.port}) does not match API base URL origin (#{base_uri.scheme}://#{base_uri.host}:#{base_uri.port}). Refusing to send credentials."
      end
      request = Net::HTTP::Get.new(uri)
      request["Authorization"] = "Bearer #{@api_key}" if @api_key
      execute_with_retry(uri, request)
    ...
    def post_multipart(path, fields:, file_field:, filename:, content:, content_type: nil)
      uri = URI("#{@base_url}#{path}")
      boundary = "----SitecrawlBoundary#{SecureRandom.hex(16)}"
    ...
    def execute_with_retry(uri, request, request_builder: nil)
      ...
      loop do
        ...
        if code >= 400 && code < 500 && code != 408 && code != 409
          raise SitecrawlError.new(error_message, status_code: code, error_code: error_code)
        end
        ...
        raise SitecrawlError.new("Request failed: #{e.message}")
      end
    end
