...
module Sitecrawl
  ...
  class Client
    DEFAULT_API_URL = "https://api.sitecrawl.dev"
    ...
    # Creates a new Sitecrawl client.
    #
    # @param api_key [String, nil] API key (falls back to SITECRAWL_API_KEY env var)
    ...
    def initialize(
      ...
      resolved_key = api_key || ENV["SITECRAWL_API_KEY"]
      ...
      resolved_url = api_url || ENV["SITECRAWL_API_URL"] || DEFAULT_API_URL
      unless resolved_url.match?(%r{\Ahttps?://}i)
        raise SitecrawlError, "API URL must be a fully qualified HTTP or HTTPS URL (got: #{resolved_url})."
      end
    ...
    def scrape(url, options = nil)
      ...
      body["origin"] ||= "ruby-sdk@#{Sitecrawl::VERSION}"
    ...
    def search_papers(query, options = {})
      @http.get("/v2/search/research/papers#{query(options.merge("query" => query, "origin" => "ruby-sdk@#{Sitecrawl::VERSION}"))}")
    end
    ...
    def read_paper(paper_id, query_text, options = {})
      ...
      @http.get("#{path}#{query(options.merge("query" => query_text, "origin" => "ruby-sdk@#{Sitecrawl::VERSION}"))}")
    end
    ...
    def related_papers(paper_id, intent, options = {})
      ...
      @http.get("#{path}#{query(options.merge("intent" => intent, "origin" => "ruby-sdk@#{Sitecrawl::VERSION}"))}")
    end
    ...
    def search_github(query_text, options = {})
      @http.get("/v2/search/research/github#{query(options.merge("query" => query_text, "origin" => "ruby-sdk@#{Sitecrawl::VERSION}"))}")
    end
    ...
    def parse(file, options = nil)
      ...
      unless file.is_a?(Models::ParseFile)
        raise ArgumentError, "File must be a Sitecrawl::Models::ParseFile"
      end
    ...
    def agent(options, poll_interval: DEFAULT_POLL_INTERVAL, timeout: DEFAULT_JOB_TIMEOUT)
      ...
      raise SitecrawlError, "Agent start did not return a job ID" if start.id.nil?
